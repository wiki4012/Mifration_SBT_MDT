...
function chartList(from, to, teamName) {
  const API = "/XMII/Illuminator?QueryTemplate=SBT_MDT%2FQuery%2FTRIGGER_LOGIC%2FGET_OPERATIONS%2FQRY%2FSELECT_DAILY_STEP_TT_V1";
      ...
      ajaxCall(
        ...
        function (result) {
          const dataList = result.Rowsets.Rowset
            ? result.Rowsets.Rowset[0].Row
            : [];
          console.log("ajax-graphValue", dataList);
...
console.log('Graph-ID', index);
                ...
                parseFloat(charts[index].UPPER_LIMIT)
                  ...
                  : "";
                //console.log(charts);
               if (charts[index].UPDATE_FREQ == 'week') //charts[index].VALUE will be undefined for Daily Cases
								{
									if (parseFloat(charts[index].LOWER_LIMIT) > parseFloat(charts[index].UPPER_LIMIT))
									{
										const colorCode =
											parseFloat(charts[index].VALUE) <= parseFloat(charts[index].UPPER_LIMIT) ?
											"rgb(33, 140, 116,0.2)" :
											(parseFloat(charts[index].VALUE) < parseFloat(charts[index].LOWER_LIMIT) && parseFloat(charts[index].VALUE) >= parseFloat(charts[index].UPPER_LIMIT) ? "rgb(255, 177, 66, 0.2)" : "rgb(179, 57, 57, 0.2)");
										...
										const borderCode =
											parseFloat(charts[index].VALUE) <= parseFloat(charts[index].UPPER_LIMIT) ?
											"rgb(33, 140, 116,1)" :
											(parseFloat(charts[index].VALUE) < parseFloat(charts[index].LOWER_LIMIT) && parseFloat(charts[index].VALUE) >= parseFloat(charts[index].UPPER_LIMIT) ? "rgb(255, 177, 66, 1)" : "rgb(179, 57, 57, 1)");
										charts[index].barColor ?
									...
									{
										const colorCode =
											parseFloat(charts[index].VALUE) <= parseFloat(charts[index].LOWER_LIMIT) ?
											"rgb(179, 57, 57, 0.2)" :
											(parseFloat(charts[index].VALUE) < parseFloat(charts[index].UPPER_LIMIT) && parseFloat(charts[index].VALUE) >= parseFloat(charts[index].LOWER_LIMIT) ? "rgb(255, 177, 66, 0.2)" : "rgb(33, 140, 116,0.2)");
										charts[index].barColor ?
											charts[index].barColor.push(colorCode) :
											"";
										const borderCode =
											parseFloat(charts[index].VALUE) <= parseFloat(charts[index].LOWER_LIMIT) ?
											"rgb(179, 57, 57, 1)" :
											(parseFloat(charts[index].VALUE) < parseFloat(charts[index].UPPER_LIMIT) && parseFloat(charts[index].VALUE) >= parseFloat(charts[index].LOWER_LIMIT) ? "rgb(255, 177, 66, 1)" : "rgb(33, 140, 116,1)");
										charts[index].barColor ?
											charts[index].borderColor.push(borderCode) :
											"";
									}
...
function generateChart(id, charter, list) {
  ...
  var chartInstance = new Chart(canvas, {
    ...
    options: {
      canvas: { //url('/XMII/CM/AP_SBT_MDT/CMN/charter.png')
